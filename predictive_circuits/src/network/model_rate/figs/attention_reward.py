...
class Attention(BaseModel):
    ...
    def process_misc(self, misc, i):
        ...
        if self.behaviour:
            ...
            if lick:
                ...
                mask_1 = self.neuron_params['dSpike_prob'] == reward_reinforce
                mask_2 = (self.f > 0.2) if reward else np.zeros(self.num_neurons, dtype=bool)
                mask = mask_1 + mask_2
                self.neuron_params['dSpike_prob'][mask] = reward_reinforce 
    ...
    def PP_generation(self, i, self_gen=False, PPs=None):
        ...
        dSpike_prob[self.PP_attempt] = self.neuron_params['dSpike_prob'][self.PP_attempt]
...
if __name__ == "__main__":
    ...
    neuron_params = {
        ...
        'norm': True,
        'dSpike_prob': 0.8 * np.ones(num_neurons)
    }
    sim_params = {
        "dt": 1,  # in ms
    ...
    neuron.reward_neurons = np.zeros(num_neurons)
    ...
    for i in range(nums):
        ...
        if i  == 0:
            neuron.behaviour = False
            mask = neuron.neuron_params['dSpike_prob'] != reward_reinforce
            neuron.neuron_params['dSpike_prob'][mask] = 0.05
            ...
            neuron.neuron_params['dSpike_prob'] = np.ones(num_neurons) * 0.8 
    ...
    with open('data/attention_reward_day_1.pkl', 'wb') as f:
        pickle.dump(day_1, f)
    with open('data/attention_reward_day_2.pkl', 'wb') as f:
        pickle.dump(day_2, f)
