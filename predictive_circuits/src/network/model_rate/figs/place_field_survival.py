...
class PlaceFieldFormation(BaseModel):
    ...
    def process_misc_after(self, misc_after):
        ...
        if misc_after.get('decay', False):
            ...
            #print(PP_mask.sum())
            #self.neuron_params['tau_acc'][self.PP_onset] *= np.exp(-0.1)
            #self.neuron_params['tau_acc'][~self.PP_onset] = rng.uniform(low=0, high=0.6, size=int((~self.PP_onset).sum()))
            #self.neuron_params['tau_acc'][self.PP_onset] *= np.exp(-0.2)
            #self.neuron_params['tau_acc'][self.PP_onset] *= np.exp(-0.1)
            #self.neuron_params['tau_acc'][~self.PP_onset] = rng.uniform(low=0, high=0.5, size=int((~self.PP_onset).sum()))
            #self.neuron_params['tau_acc'][~self.PP_onset] = rng.exponential(scale=0.15, size=int((~self.PP_onset).sum()))
            #self.neuron_params['tau_acc'][~self.PP_onset] = rng.choice(a=vals, size=int((~self.PP_onset).sum()), p=density) / max_val
            #self.neuron_params['tau_acc'][~self.PP_onset] = rng.uniform(low=0, high=0.6, size=int((~self.PP_onset).sum()))
            ...
            self.neuron_params['decay_fac'][mask_2] = np.exp(-0.05)
            self.neuron_params['tau_acc'][~PP_mask] = rng.choice(a=a, size=int((~PP_mask).sum()), p=p)
            self.neuron_params['tau_acc'][PP_mask] *= self.neuron_params['decay_fac'][PP_mask]
...
if __name__ == "__main__":
    ...
    num_pcs = 200
    #tau_acc = rng.exponential(scale=100, size=num_neurons)
    #tau_acc = rng.uniform(low=0, high=0.5, size=num_neurons)
    #tau_acc = rng.uniform(low=75, high=75, size=num_neurons)
    tau_acc = rng.exponential(scale=0.15, size=num_neurons)
    tau_acc = rng.choice(a=a, size=num_neurons, p=p)
    ...
    for i in range(nums):
        ...
        if i  == 0:
            ...
            PPs = -1 * np.ones(num_neurons)
            rnd_neurons = np.random.choice(np.arange(0, num_neurons, dtype=int)[elig_mask], num_pcs - num_neurons_with_PC, replace=False)
            neurons_with_random_PC.append(rnd_neurons)
            PP_times = np.random.choice(time_steps, num_pcs - num_neurons_with_PC)
            PPs[rnd_neurons] = PP_times
            ...
            if False:
                ...
                num_neurons_with_PP = int(np.minimum(num_neurons_with_PP, num_pcs))
