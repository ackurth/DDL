    ...
    def run_passive(self, state, timesteps=0):
        ...
        for i in range(timesteps):
            w_prox = ((1 - self.sim_params['dt'] * speed) * w_prox
                           ...
                           + self.rng.normal(0, self.neuron_params['w_sum'] / self.num_inp / 2 * np.sqrt(2 * speed)))
...
if __name__ == "__main__":
    ...
    num_neurons = 100
    tau_acc = rng.uniform(low=999, high=1000, size=num_neurons)
    ...
    for i in range(nums):
        ...
        if i > 1:
            state = neuron.run_passive(state, 5 * int(1e4))
